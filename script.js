...
const fetchCryptoData = async () => {
    try {
        ...
        populateCryptoTable(staticData); // Fallback to static data on error
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
        const price = (Math.random() * (0.5 - 0.4) + 0.4).toFixed(3);
        bcoinPrice.textContent = `$${price}`;

        const fluctuation = (Math.random() * 6 - 3).toFixed(1);
        priceFluctuation.textContent = `24h Change: ${fluctuation}%`;

        priceFluctuation.style.color = fluctuation > 0 ? '#00b400' : '#ff0000';
    }, 3000);
    ...
    document.getElementById('calculate-btn').addEventListener('click', () => {
        ...
        if (investmentAmount && !isNaN(investmentAmount)) {
            const bcoinPrice = 0.459;
            const amountOfBcoin = (investmentAmount / bcoinPrice).toFixed(4);
            document.getElementById('result').textContent = 
                `You can buy ${amountOfBcoin} BCoin for $${investmentAmount}.`;
        } else {
